...
/**
 * Parser for the Optical and Imaging Devices table.
 *
 ...
 */
export class GearElectronicOpticalParser extends BaseItemParser {
  static GEAR_TYPE = "ELECTRONICS";
  static GEAR_SUBTYPE = "OPTICAL";
  ...
  parse() {
    ...
    const headerIndex = lines.findIndex((line) => GearElectronicOpticalParser.TABLE_HEADER.test(this.normalizeLine(line)));

    if (headerIndex < 0) {
      ui.notifications?.warn('No optical-device table header found for "DEVICE CAPACITY AVAIL COST".');
      return [];
    }
    ...
    if (!rows.length) {
      ui.notifications?.warn("No optical or imaging device rows were recognized below the table header.");
    }
  ...
  parseTableRows(lines, startIndex) {
    ...
    while (index < lines.length) {
      ...
      // PDF extraction can wrap names such as "Binoculars, optical" and
      // "Mage sight goggles" over two lines. Parse up to three joined lines,
  ...
  parseTableRow(line) {
    ...
    if (!normalized || GearElectronicOpticalParser.TABLE_HEADER.test(normalized)) return null;
  ...
  toFoundryItem({ row, capacity = 0, rangedCapacity = false, description = "", warnings = [] } = {}) {
    ...
    const baseName = row?.name || "Unnamed Optical Device";
    ...
    return {
      ...
      system: {
        ...
        wild: false,
        pool: 0,
        type: GearElectronicOpticalParser.GEAR_TYPE,
        subtype: GearElectronicOpticalParser.GEAR_SUBTYPE,
        count: 0,
        countable: false,
